...
function handleRouting() {
  ...
  if (pageId) {
    ...
    if (target) {
      ...
      console.warn(
        `[Router] No element with id="${pageId}" for hash "${hash}"`,
      );
...
function syncAuthClasses() {
  ...
  // Show/hide nav sections based on auth state
  ...
  // Update dropdown button label to reflect the user's actual role
  const dropdownBtn = document.getElementById("adminDropdown");
  if (dropdownBtn) {
    dropdownBtn.textContent = currentUser ? currentUser.role : "Account";
  }
}
...
function handleRegister() {
  ...
  if (verifyMsg)
    verifyMsg.textContent = `✅ A verification link has been sent to ${email}.`;
...
function handleVerify() {
  ...
  localStorage.setItem("show_verified_banner", "1");
...
function renderProfile() {
  ...
  document
    .querySelector("#profile-page .btn-outline-primary")
...
function renderAccountsList() {
  ...
  tbody.innerHTML = window.db.accounts
    .map(
      (acc) => `
    <tr>
      ...
      <td>
        <button class="btn btn-outline-primary btn-sm me-1"
          onclick="openEditAccountModal('${acc.email}')">Edit</button>
        <button class="btn btn-outline-warning btn-sm me-1"
          onclick="openResetPwModal('${acc.email}')">Reset PW</button>
        <button class="btn btn-outline-danger btn-sm"
          onclick="openDeleteAccountModal('${acc.email}')">Delete</button>
      </td>
    </tr>
  `,
    )
...
function openResetPwModal(email) {
  document.getElementById("reset-pw-email-label").textContent = email;
  document.getElementById("modal-new-password").value = "";
  document.getElementById("resetPwModal").dataset.targetEmail = email;
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("modal-reset-pw-btn")
    .addEventListener("click", () => {
      const email = document.getElementById("resetPwModal").dataset.targetEmail;
      ...
      bootstrap.Modal.getInstance(
        document.getElementById("resetPwModal"),
      ).hide();
...
function openEditAccountModal(email) {
  ...
  document.getElementById("modal-acc-firstname").value = acc.firstName;
  document.getElementById("modal-acc-lastname").value = acc.lastName;
  document.getElementById("modal-acc-email").value = acc.email;
  document.getElementById("modal-acc-password").value = "";
  document.getElementById("modal-acc-role").value = acc.role;
  document.getElementById("modal-acc-verified").checked = acc.verified;

  // Store which account we're editing
  document.getElementById("editAccountModal").dataset.editingEmail = email;
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("modal-save-account-btn")
    .addEventListener("click", () => {
      const originalEmail =
        document.getElementById("editAccountModal").dataset.editingEmail;
      const acc = window.db.accounts.find((a) => a.email === originalEmail);
      if (!acc) return;

      const firstName = document
        .getElementById("modal-acc-firstname")
        .value.trim();
      const lastName = document
        .getElementById("modal-acc-lastname")
        .value.trim();
      const email = document
        .getElementById("modal-acc-email")
        .value.trim()
        .toLowerCase();
      ...
      if (currentUser && currentUser.email === originalEmail) currentUser = acc;
...
function openDeleteAccountModal(email) {
  ...
  document.getElementById("deleteAccountModal").dataset.targetEmail = email;
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("modal-confirm-delete-account-btn")
    .addEventListener("click", () => {
      const email =
        document.getElementById("deleteAccountModal").dataset.targetEmail;
...
function closeAccountForm() {
  document.querySelector("#accounts-page .card").classList.add("d-none"); // 👈 hide it
...
function saveAccount() {
  ...
  saveToStorage();
  editingAccountEmail = null;
  document.querySelector("#accounts-page .card .card-header").textContent =
    "Add/Edit Account";
  closeAccountForm();
  renderAccountsList();
...
function renderDepartmentsList() {
  ...
  tbody.innerHTML = window.db.departments
    .map(
      (dept) => `
    <tr>
...
// Opens the modal and populates fields
function editDepartment(id) {
  ...
  document.getElementById("editDepartmentDescription").value =
    dept.description ?? "";
...
// Wires up the Save button — call this once on DOMContentLoaded
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("saveDepartmentBtn").addEventListener("click", () => {
    const rawId = document.getElementById("editDepartmentId").value;
    ...
    // Compare as both string and number to handle either id type
    const dept = window.db.departments.find((d) => d.id == rawId);
    if (!dept) {
      console.error("Department not found for id:", rawId);
      return;
    }
  ...
  // Clear validation state when the modal is reopened
  document
...
// Opens the modal, showing either a warning or a confirmation
function deleteDepartment(id) {
  const dept = window.db.departments.find((d) => d.id == id);
  if (!dept) return;

  document.getElementById("deleteDepartmentId").value = dept.id;
  ...
  if (inUse) {
    // Block deletion — show warning, disable confirm button
    confirmMsg.classList.add("d-none");
    warnText.textContent = `Cannot delete "${dept.name}" — it has employees assigned to it.`;
    warnMsg.classList.remove("d-none");
    deleteBtn.disabled = true;
  } else {
    // Safe to delete — show confirmation, enable confirm button
    warnMsg.classList.add("d-none");
    confirmMsg.textContent = `Are you sure you want to delete "${dept.name}"? This action cannot be undone.`;
...
// Wire up the confirm button — call once on DOMContentLoaded
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("confirmDeleteDepartmentBtn")
    .addEventListener("click", () => {
...
// Wire up the Add button — call once on DOMContentLoaded
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Clear form fields and validation state every time the modal opens
...
function renderEmployeesTable() {
  ...
  if (window.db.employees.length === 0) {
    ...
    tbody.innerHTML = window.db.employees
      .map((emp) => {
...
function openEmployeeForm(employeeIdToEdit = null) {
  ...
  if (employeeIdToEdit) {
    const emp = window.db.employees.find(
      (e) => e.employeeId === employeeIdToEdit,
    );
    if (!emp) return;
    ...
    document.querySelector("#employee-form-card .card-header").textContent =
      "Edit Employee";
  } else {
  ...
  employeeFormCard().scrollIntoView({ behavior: "smooth" });
}
...
function saveEmployee() {
  ...
  if (!employeeId || !userEmail || !position || !deptId) {
    showError(
      "employees-page",
      "Employee ID, email, position, and department are required.",
    );
    return;
  }
  ...
  if (!linkedAccount) {
    showError(
      "employees-page",
      `No account found for email "${userEmail}". Create the account first.`,
    );
    return;
  }
  ...
  if (editingEmployeeId) {
    const emp = window.db.employees.find(
      (e) => e.employeeId === editingEmployeeId,
    );
    if (!emp) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ── Registration ──────────────────────────
  document
    .querySelector("#register-page .btn-success")
    ?.addEventListener("click", handleRegister);

  document
    .querySelector("#register-page .btn-outline-secondary")
    ?.addEventListener("click", () => navigateTo("#/"));

  // ── Verify Email ──────────────────────────
  document
    .querySelector("#verify-page .btn-success")
    ?.addEventListener("click", handleVerify);

  document
    .querySelector("#verify-page .btn-outline-secondary")
    ?.addEventListener("click", () => navigateTo("#/"));

  // ── Login ─────────────────────────────────
  document
    .querySelector("#login-page .btn-primary")
    ?.addEventListener("click", handleLogin);

  document
    .querySelector("#login-page .btn-outline-secondary")
    ?.addEventListener("click", () => navigateTo("#/"));
  ...
  // ── Get Started button ────────────────────
  document
    .querySelector("#home-page .btn-primary")
    ?.addEventListener("click", () =>
  ...
  // ── Accounts page ─────────────────────────
  document
    .querySelector("#accounts-page > .d-flex .btn-success")
    ?.addEventListener("click", () => openAccountForm(null));

  document
    .querySelector("#accounts-page .card .btn-primary")
    ?.addEventListener("click", saveAccount);

  document
    .querySelector("#accounts-page .card .btn-secondary")
    ?.addEventListener("click", () => {
      editingAccountEmail = null;
      document.querySelector("#accounts-page .card .card-header").textContent =
        "Add/Edit Account";
  ...
  document
    ...
    ?.addEventListener("click", closeAccountForm); // 👈 replaces the old inline handler

  // ── Departments page ──────────────────────
  document
    .querySelector("#departments-page .btn-success")
    ?.addEventListener("click", addDepartment);

  // ── Employees page ────────────────────────
  document
    .querySelector("#employees-page > .container > .d-flex .btn-success")
    ?.addEventListener("click", () => openEmployeeForm(null));

  document
    .querySelector("#employees-page .card .btn-primary")
    ?.addEventListener("click", saveEmployee);

  document
    .querySelector("#employees-page .card .btn-outline-secondary")
    ?.addEventListener("click", () => {
      editingEmployeeId = null;
      document.querySelector("#employees-page .card .card-header").textContent =
        "Add/Edit Employees";
      removeAlert("employees-page");
    });
